...
exports.consultarVentas = async function (oFiltro) { 
    ...
    try {  
        var aFiltroVenta = [];
        var sQueryVenta = `select v.*, ssve.\"CodEstado\", ssve.\"Estado\" from venta.venta v inner join (
            select sve.* from venta.venta_estado sve inner join (
            select Max(ve.\"Id\") as \"Id\", ve.\"VentaId\" from venta.venta_estado ve where ve.\"EstadoId\" = 1 group by ve.\"VentaId\"  
            ) xsv
            on sve.\"Id\" = xsv.\"Id\" 
            ) ssve
            on v.\"Id\" = ssve.\"VentaId\"
            where v.\"EstadoId\" = 1 `;
        ...
        if(oFiltro.sUsuarioCreador !== undefined){
            sQueryVenta = sQueryVenta + " and v.\"UsuarioCreador\" = ? ";
            aFiltroVenta.push(oFiltro.sUsuarioCreador);
        } 
